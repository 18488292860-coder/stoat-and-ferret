...
Cross-reference backlog items referenced in feature requirements with their actual status for stoat-and-ferret version v004, and complete any still-open items that were implemented.
...
For each backlog item that is still "open" but was referenced by a completed feature, execute:
`complete_backlog_item(project="stoat-and-ferret", item_id="BL-XXX", version="v004", theme="<theme-name>")`

Record: item ID, action taken, result.

### 4. Check for Orphaned Items
Call `list_backlog_items(project="stoat-and-ferret", status="open")` and check if any open items reference v004 in their description or notes but were not found in requirements files.

## Output Requirements

Save outputs to comms/outbox/exploration/v004-retro-003-backlog/:
...
Do NOT commit — the calling prompt handles commits. Results folder: v004-retro-003-backlog.